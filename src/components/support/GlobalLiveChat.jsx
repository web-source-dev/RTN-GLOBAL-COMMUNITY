  ...
  const initializeChat = async () => {
    try {
      ...
      newSocket.on('messageEdited', (data) => {
        ...
        // Extract message data safely, handling different possible formats
        const messageId = data._id || data.message?._id;
        const content = data.content || data.message?.content;
        const editHistory = data.editHistory || data.message?.editHistory;
        const roomId = data.roomId || data.message?.roomId || data.chatRoom;
        
        if (!messageId || !roomId || !chatRoom || roomId !== chatRoom._id) {
          console.warn('Skipping message edit - invalid data or wrong room', { messageId, roomId });
          return;
        }
        
        console.log(`Processing edit for message ${messageId} in room ${roomId}`);
        
        // Update messages with edited content
        setMessages(prevMessages => {
          return prevMessages.map(msg => {
            if (msg._id === messageId) {
              console.log('Updating edited message:', messageId);
      ...
      newSocket.on('messageDeleted', (data) => {
        ...
        // Extract message data safely from different possible formats
        const messageId = data.messageId || data._id || data.message?._id;
        const roomId = data.roomId || data.message?.roomId || data.chatRoom;
        
        if (!messageId || !roomId || !chatRoom || roomId !== chatRoom._id) {
          console.warn('Skipping message delete - invalid data or wrong room', { messageId, roomId });
          return;
        }
        
        console.log(`Processing delete for message ${messageId} in room ${roomId}`);
        
        // Update the message in the UI
        setMessages(prevMessages => {
          return prevMessages.map(msg => {
            if (msg._id === messageId) {
              console.log('Marking message as deleted:', messageId);
              // Create a new object to ensure React detects the change
              return {
                ...msg,
                deleted: true,
